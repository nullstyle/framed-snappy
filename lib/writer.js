var snappy, fs, ref$, map, sum, getBufferOfSize, maskedCrcForBuffer, CRC32_SIZE, FRAME_IDS, STREAM_IDENTIFIER, FRAME_HEADER_SIZE, TARGET_FRAME_SIZE, writeStreamId, writeChunkId, writeChunkLength, writeChunkData, writeCrc, writeCompressedChunk, writeUncompressedChunk, Writer;
...
writeUncompressedChunk = function(output, crc, uncompressedData){
  writeChunkId(output, "uncompressedData");
  writeChunkLength(output, CRC32_SIZE + uncompressedData.length);
  writeCrc(output, crc);
  return writeChunkData(output, uncompressedData);
};
Writer = (function(){
  ...
  prototype._finishFrame = function(cb){
    ...
    return snappy.compress(toWrite, function(err, compressedData){
      var sizeSavings, outputBufferSize, outputBuffer;
      if (err != null) {
        return cb(err);
      }
      sizeSavings = 1.0 - compressedData.length / toWrite.length;
      if (sizeSavings > 0.1) {
        outputBufferSize = FRAME_HEADER_SIZE + CRC32_SIZE + compressedData.length;
        outputBuffer = new Buffer(outputBufferSize);
        writeCompressedChunk(outputBuffer, crc, compressedData);
      } else {
        outputBufferSize = FRAME_HEADER_SIZE + CRC32_SIZE + toWrite.length;
        outputBuffer = new Buffer(outputBufferSize);
        writeUncompressedChunk(outputBuffer, crc, toWrite);
      }
      return fs.write(this$.file, outputBuffer, 0, outputBufferSize, null, function(err, bytesWritten, buffer){
        if (err != null) {
          return cb(err);
        }
